...
const CarouselNewRedux = () => {
    ...
    // FILTER HOT COLLECTION TO ONLY INCLUDE ENTITIES WHERE AUTHOER WEXISTS + PREVIEW IMAGE
    const collectionWhereAuthorExists = nfts.filter(x => {
        return x.author && x.preview_image
    })

    return (
        <div className='nft'>
          <Slider {...carouselNew}>
          {collectionWhereAuthorExists && collectionWhereAuthorExists.map( (nft, index) => (
